...
new_y = transpose(new_y);

J = (1/m)*sum(sum((-(new_y).*log(a3)) - ((1-new_y).*log(1-a3))));

% regularizing term
R = (lambda/(2*m)) * (sum(sum(Theta1(:,2:end).*Theta1(:,2:end)))+...
...
for t = 1:m
    ...
    delta_3 = a3(:,t) - new_y(:,t);
    z2_1 = [ones(1, m); z2];
    delta_2 = (transpose(Theta2)*delta_3).* sigmoidGradient(z2_1(:,t));

    Theta1_grad = Theta1_grad + delta_2(2:end)*transpose(a1_1);
    Theta2_grad = Theta2_grad + delta_3*transpose(a2_1);
